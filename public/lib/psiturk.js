...
var PsiTurk = function (uniqueId, adServerLoc, mode) {
  ...
  var TaskData = Backbone.Model.extend({
    ...
    initialize: function () {
      ...
      this.listenTo(
        Backbone.Notifications,
        "_psiturk_gainedfocus",
        function () {
          this.addEvent("focus", "on");
        }
      );
      this.listenTo(Backbone.Notifications, "_psiturk_windowresize", function (
        newsize
      ) {
  ...
  var Instructions = function (parent, pages, callback) {
    ...
    var loadPage = function () {
      ...
      if (currentscreen != 0) {
        ...
        $(".instructionsnav").on(
          "click.psiturk.instructionsnav.prev",
          ".previous",
          function () {
            prevPageButtonPress();
          }
      ...
      $(".instructionsnav").on(
        "click.psiturk.instructionsnav.next",
        ".continue",
        function () {
          nextPageButtonPress();
        }
    ...
    self.getIndicator = function () {
      return {
        currently_viewing: {
          ...
          template: pages[currentscreen],
        },
        instruction_deck: {
          total_pages: instruction_pages.length,
          templates: instruction_pages,
        },
      };
    };
  ...
  self.getPage = function (pagename) {
    if (!(pagename in self.pages)) {
      throw new Error(
        ["Attemping to load page before preloading: ", pagename].join("")
      );
    }

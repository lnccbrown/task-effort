...
Github now requires all URLS to use HTTPS. Because some of the dependencies in this packed are pinned to older versions we need to run
...
3. Navigate to the task-effort directory and then install the dependencies. You may first need to install Node.js (https://nodejs.org/en/download/) before being able to use npm commands in the terminal.
...
6. In rare occasions you may need to build the package locally (this is typically only necessary in the github actions). To build:
...
1. Clone the repo and create a new feature branch off `main`.
...
3. Submit a [pull request](https://help.github.com/en/articles/creating-a-pull-request) to the `develop` branch. Add a reviewer.
